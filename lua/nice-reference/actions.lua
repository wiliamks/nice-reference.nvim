...
M.preview = function(_, item, _)
    ...
    if ok then
        M.close()
        local preview_window = lib.open_floating_win('file://' .. item.filename, { item.lnum, item.col - 1 })

        local function map_close_window(keys)
            for i = 1, #keys do
                vim.keymap.set(
                    "n",
                    keys[i],
                    ...
                    { silent = true, noremap = true, buffer = vim.api.nvim_win_get_buf(preview_window) }
                )
            end
        end

        map_close_window({ '<C-c>', 'q' })
    else
